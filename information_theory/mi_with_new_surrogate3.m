 ...
 [in,on]=inpolygon(xq,yq,xx(:,box_no),yy(:,box_no));
  ...
  for tt=1:NT
    ...
    if tt==1;
     ...
     mean_val=mean(BZ1(:));
   curl_grid(tt,:)=((BZ1(A1(1:no_points))-mean_val).^2);
...
upper_lim_cor=mean_sur_cor+std_sur_cor;
lower_lim_cor=mean_sur_cor-std_sur_cor;
...
%getting the significance value for the maximum mi value time series 
 ...
 y_value=yy(I:nn+I-1,no_points+1:end);
